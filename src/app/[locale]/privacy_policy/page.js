...
export async function generateMetadata() {   
    ...
    return {
        ...
        description: t('privacy_policy_subtext')
    }
}
...
export default async function page() {
    const t = await getTranslations()
    const contents = await WebServices.getContentbyId({id:'6'})
    return (
        <>
            <Header />
            {/* <Breadcrumb title={t('privacy_policy')} subtitle={t('privacy_policy_subtext')} img={contents.data.imageUrl} /> */}
            <Breadcrumb title={t('privacy_policy')} subtitle={t('privacy_policy_subtext')} img={'/img/delete/br1.webp'} />
            <div className='w-1270 mx-auto flex flex-col gap-32 text-lg leading-snug sm:w-full sm:gap-12 sm:p-5 sm:text-sm moviedetail'>
                <div className='w-full flex flex-col gap-5'>
                    <Title title={t('privacy_policy')} subtitle={t('privacy_policy_subtext')} />
